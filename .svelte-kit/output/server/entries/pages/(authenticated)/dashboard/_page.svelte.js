...
const SvelteTable = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="prose max-w-none " style="display: flex; justify-content: space-between; align-items: center; "><div><h2>Batches</h2></div>
    <div style="display: flex; align-items: center;"><div><input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text" placeholder="Filter by Batch Name"${add_attribute("value", batchFilter, 0)}></div>    
      ${``}
      <button class="btn btn-info"${add_attribute("style", `margin-left: 1rem; background: red`, 0)}>+ Add Batch</button></div></div>
...
const css = {
  code: "button.svelte-1mu6qum{padding:10px 20px;font-size:16px;font-weight:bold;border:none;background-color:#f5f5f5;color:#555;cursor:pointer;transition:background-color 0.3s ease}button.selected.svelte-1mu6qum{background-color:#2196f3;color:#fff}",
  map: null
};
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  $$result.css.add(css);
  return `<div class="prose" style="display:flex; justify-content:center; flex-direction:column; margin-bottom:3%; "><h1>Dashboard</h1></div>
<div class="tab"><button class="${["svelte-1mu6qum", "selected"].join(" ").trim()}">Batches</button>
  <button class="${["svelte-1mu6qum", ""].join(" ").trim()}">Employees</button></div>
